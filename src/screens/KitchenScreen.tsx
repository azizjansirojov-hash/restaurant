  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>No active tickets. Paid orders appear here.</Text>
        }
        renderItem={({ item }) => (
          <View style={styles.ticket}>
            ...
            {NEXT_LABEL[item.status] && (
              <Button
                label={bumping === item.id ? 'Updating…' : NEXT_LABEL[item.status]!}
            ...
            {cancelId === item.id && (
              <View style={{ gap: 8, marginTop: 10 }}>
                ...
                <Button
                  ...
                  variant="secondary"
                  onPress={() => {
                    cancelOrder(item.id, reason || 'Cancelled by kitchen');
                    setCancelId(null);
                    setReason('');
                  }}
                />
